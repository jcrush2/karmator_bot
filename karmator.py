...
def commands(msg, text):
	...
	if msg.text.lower() in ['язабан']:
		if msg.reply_to_message:
			bot.send_message(msg, f"{msg.from_user.first_name} <a href='tg://user?id=55910350'>❤</a>dfsd", parse_mode="HTML")
			bot.send_poll(msg.chat.id, f'{msg.reply_to_message.from_user.first_name} Это опрос?', ['Да', 'Нет', 'Не знаю'],is_anonymous=False,close_date=60)
		else:
			return
